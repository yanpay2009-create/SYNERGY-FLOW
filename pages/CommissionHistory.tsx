            ...
            <div className="flex-1 px-6 flex flex-col items-center justify-center animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 font-medium mb-8 text-center uppercase tracking-widest">
                    {selectedTx.type === 'Withdrawal' 
                        ...
                        : (selectedTx.type === 'Withdrawal' ? 'Withdrawal processed successfully' : 'Commission earned successfully')}
                </p>
    ...
    <div className="pb-24 pt-0 px-4 max-w-md mx-auto min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors relative">
      {isVerifyingPin && (
          <div className="fixed inset-0 z-[110] bg-white dark:bg-gray-900 flex flex-col items-center justify-center px-6 transition-all duration-300">
              ...
              <h2 className="text-2xl font-black text-gray-900 dark:text-white mb-2">{showPinSuccess ? 'Success!' : (pinFlow === 'verify' ? 'Enter Security PIN' : 'Set Security PIN')}</h2>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-10 text-center">
                {pinFlow === 'verify' ? 'Enter your 6-digit security PIN to continue.' : 'Create a 6-digit PIN to protect your funds.'}
              </p>
                ...
                >
                    <div className="flex items-center space-x-3">
                        ...
                        <div className="min-w-0">
                            ...
                            {tx.orderId && (
                                <div className="mt-2">
                                    {allOrders.find(o => o.id === tx.orderId) && (
                                        <div className="flex flex-wrap gap-1.5">
                                            {allOrders.find(o => o.id === tx.orderId)?.items.map((item, i) => (
                                                <div key={i} className="flex items-center space-x-1.5 bg-gray-50 dark:bg-gray-700/50 p-1 pr-2 rounded-lg border border-gray-100 dark:border-gray-600">
                                                    <img 
                                                        src={item.image} 
                                                        alt={item.name} 
                                                    ...
                                                    />
                                                    <div className="min-w-0">
                                                        <p className="text-[8px] font-bold text-gray-700 dark:text-gray-300 truncate max-w-[80px]">
                                                            {item.name}
                                                        </p>
                                                        <p className="text-[7px] text-gray-400 font-black uppercase">x{item.quantity}</p>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
