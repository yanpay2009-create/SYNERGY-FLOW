...
export const Withdraw: React.FC = () => {
  const { user, bankAccounts, selectedBankId, selectBank, withdrawFunds, kycStatus, isSecurityUnlocked, setIsSecurityUnlocked, updateUserSecurity, t, language, systemSettings } = useApp();
  ...
  const handlePinInput = (value: string) => {
    ...
    if (value.length === 6) {
      if (pinFlow === 'verify') {
        ...
        } else { setPinError(true); setTimeout(() => setPin(''), 500); }
        ...
        } else { setPinError(true); setTimeout(() => { setPin(''); setTempPin(''); setPinFlow('setup'); }, 500); }
      }
    }
  };
  ...
  const handleWithdraw = (e: React.FormEvent) => {
    ...
    if (!isValidAmount) { alert("Min withdrawal is ฿100."); return; }
  ...
  const handleDownloadSlip = async () => {
    if (slipRef.current) {
        try {
            ...
            alert('Could not generate slip image. Please try again or take a screenshot.');
        }
    }
  };
            ...
            <div className="max-w-md mx-auto px-6">
                ...
                >
                    {systemSettings.slipBackground && (
                        <img 
                            src={systemSettings.slipBackground} 
                            alt="Background" 
  ...
  return (
    <div className="pb-24 pt-8 px-4 max-w-md mx-auto min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300 relative font-sans">
      {isProcessing && (
      ...
      {isVerifyingPin && (
          <div className="fixed inset-0 z-[110] bg-white dark:bg-gray-900 flex flex-col items-center justify-center px-6 transition-all duration-300">
              ...
              <h2 className="text-2xl font-black text-gray-900 dark:text-white mb-2">{showPinSuccess ? 'Success!' : 'Enter Security PIN'}</h2>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-10 text-center">Enter your 6-digit security PIN to continue.</p>
              <div className="flex space-x-4 mb-10">
                  {[...Array(6)].map((_, i) => (
                  <div key={i} className={`w-4 h-4 rounded-full transition-all duration-300 ${i < pin.length ? 'bg-synergy-blue scale-125' : 'bg-gray-200 dark:bg-gray-700'} ${pinError ? 'bg-red-50' : ''}`} />
                  ))}
              </div>
      ...
      <form onSubmit={handleWithdraw}>
        <div className="mb-8">
            ...
            {bankAccounts.length === 0 ? (
                <button type="button" onClick={() => navigate('/bank-accounts')} className="w-full py-6 border-2 border-dashed border-gray-200 dark:border-gray-700 text-gray-400 rounded-3xl flex flex-col items-center justify-center space-y-2 hover:bg-white transition">
                   <Plus size={20} />
