...
export const MyOrders: React.FC = () => {
  const { orders, updateOrderAddress, user, updateOrderStatus, cart } = useApp();
  ...
  const handleSubmitReturn = () => {
      ...
      setTimeout(() => {
          if (showReturnModal) {
              ...
              setReturnReason('');
              alert("Return request submitted successfully. Our team will review your request within 24 hours.");
          }
      }, 1500);
  };

  const handleForceCompleteReturn = (e: React.MouseEvent, orderId: string) => {
      e.stopPropagation();
      if (window.confirm("Verify return and process refund/commission deduction?")) {
          updateOrderStatus(orderId, 'Returned');
          alert("Return finalized. Commissions reversed and sales volume adjusted.");
      }
  };
                        ...
                        <div className="p-4" onClick={() => toggleExpand(order.id)}>
                            ...
                            <div className="flex space-x-2 pt-3.5 border-t border-gray-50 dark:border-gray-800">
                                {order.status === 'Delivered' ? (
                                    ...
                                    <button 
                                        onClick={(e) => handleForceCompleteReturn(e, order.id)}
