...
export const Account: React.FC = () => {
  const { user, isLoggedIn, getCommissionRate, team, kycStatus, commissions, orders, t, updateUserSecurity, isSecurityUnlocked, setIsSecurityUnlocked, logout, referrer, addReferrer, ads, products } = useApp();
  ...
  const handleAddReferrerFromModal = () => {
    if (!referrerCode) return;
    const success = addReferrer(referrerCode);
    if (success) {
        setShowReferrerModal(false);
        setReferrerError('');
        alert("Referrer linked! You can now share your profile.");
    } else {
        setReferrerError("Referrer code not found. Please try again.");
    }
  };
  ...
  return (
    <div className="pb-0 pt-0 max-w-md mx-auto min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300 relative overflow-x-hidden">
      ...
      <div className="relative z-10 bg-white dark:bg-gray-900 rounded-t-[40px] mt-32 pt-10 px-4 pb-20 shadow-[0_-15px_40px_rgba(0,0,0,0.08)] min-h-[calc(100vh-8rem)]">
          ...
          <div onClick={() => navigate('/commissions')} className="w-full text-left bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl rounded-[24px] p-6 mb-4 shadow-soft dark:shadow-none border border-white/60 dark:border-gray-700 relative overflow-hidden group transition-all duration-200 cursor-pointer active:scale-[0.98]">
          ...
          {user.tier !== UserTier.EXECUTIVE && hasPromoProducts && (
            activeAd ? (
                <div 
                  ...
                  className="w-full h-32 rounded-[24px] overflow-hidden shadow-soft mb-8 relative group cursor-pointer active:scale-[0.98] transition-all duration-500 border border-white/60 dark:border-gray-700 animate-in slide-in-from-bottom-2 duration-700"
                >
                    <img src={activeAd.image} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-1000" alt="Account Ad" />
                ...
                <div 
                  onClick={() => navigate('/promotions')}
                  className="w-full h-32 rounded-[24px] bg-gradient-to-br from-synergy-blue to-purple-800 mb-8 p-5 flex flex-col justify-center text-white shadow-soft relative overflow-hidden group cursor-pointer active:scale-[0.98] transition-all border border-white/60 dark:border-gray-700"
                >
