...
export const AdminAdManage: React.FC = () => {
  ...
  const [activePlacement, setActivePlacement] = useState<'all' | 'home' | 'feed' | 'account'>('all');
  const placements = ['all', 'home', 'feed', 'account'] as const;
  ...
  const handleAddNew = () => {
      addAd({
          ...
          active: true,
          placement: activePlacement === 'all' ? 'home' : activePlacement,
  ...
  const filteredAds = ads.filter(ad => activePlacement === 'all' || ad.placement === activePlacement);
  ...
  return (
    <div className="pb-24 pt-4 px-4 max-w-md mx-auto min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors">
      ...
      {/* Placement Tabs */}
      <div className="flex space-x-2 overflow-x-auto no-scrollbar mb-6 -mx-4 px-4">
          {placements.map(pos => (
              <button
                  key={pos}
                  onClick={() => setActivePlacement(pos)}
                      ...
                      <div className="space-y-4 mb-5">
                          <div>
                            ...
                            <div className="grid grid-cols-3 gap-2">
                                {(['home', 'feed', 'account'] as const).map(pos => (
                                    <button 
                                        key={pos}
                                        onClick={() => updateAd(ad.id, { placement: pos })}
                                        className={`py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all border ${ad.placement === pos ? 'bg-indigo-500 text-white border-indigo-500 shadow-sm' : 'bg-gray-50 dark:bg-gray-700 text-gray-400 border-transparent hover:border-gray-200'}`}
                                    >
                                        {pos}
                                    </button>
                                ))}
                          ...
                          <div>
                            ...
                            <div className="flex items-center space-x-2 bg-gray-50 dark:bg-gray-700 p-3 rounded-xl">
                                ...
                                <input 
                                    ...
                                    onChange={(e) => updateAd(ad.id, { expiryDate: e.target.value ? new Date(e.target.value).toISOString() : undefined })}
                                    className="bg-transparent w-full outline-none dark:text-white text-[10px]"
                                />
                                {ad.expiryDate && (
                                  <button 
                                    onClick={() => updateAd(ad.id, { expiryDate: undefined })}
      ...
      <div className="mt-8 p-5 bg-indigo-50 dark:bg-indigo-900/20 rounded-2xl border border-indigo-100 dark:border-indigo-800">
          ...
          <p className="text-[10px] text-indigo-500 leading-relaxed space-y-1">
            ...
            • <b>Account:</b> Appears on Account, Home (carousel), and Promotions pages.
          </p>
      </div>
    </div>
  );
