...
import { 
  ...
  QrCode, Loader2, ShieldCheck, Box, ClipboardList, Check, CheckCircle, Landmark, Banknote,
  Layout, Megaphone, Monitor, ChevronRight
} from 'lucide-react';
...
export const AdminDashboard: React.FC = () => {
  ...
  const { 
    ...
    updateFeedStatus, deleteFeedPost, systemSettings, updateSystemSettings, updateKycStatus 
  } = useApp();
  
  const [activeTab, setActiveTab] = useState<'Members' | 'Orders' | 'Withdrawals' | 'Posts' | 'KYC' | 'Settings'>('Members');
  ...
  const filteredList = useMemo(() => {
    ...
    if (activeTab === 'KYC') {
        return allTeamMembers.filter(m => {
            ...
            // In a real app, we'd check a real KYC status. For now, we'll show all members and their status.
            return matchesQuery;
        });
    }
                  ...
                  <div className="flex flex-col items-center">
                      ...
                      >
                          {systemSettings.slipBackground ? (
                              <>
                                <img src={systemSettings.slipBackground} className="w-full h-full object-cover" alt="Slip Background" />
                                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition">
                                    <Camera size={28} className="text-white" />
                                </div>
                              </>
          ...
          <div className="bg-transparent animate-in fade-in duration-500 space-y-6">
              ...
              <div className="space-y-6">
                  <div className="bg-white dark:bg-gray-800 rounded-[32px] p-6 shadow-soft dark:shadow-none border border-transparent dark:border-gray-700">
                      ...
                      >
                          {promoForm.image ? (
                              <img src={promoForm.image} className="w-full h-full object-contain p-4" alt="Promo" />
                          ) : (
            ...
            <div className="space-y-4">
                {filteredList.length === 0 ? (
                    ...
                    filteredList.map((item: any) => {
                        if (activeTab === 'Members') {
                            return (
                              <div key={item.id} className="flex items-center justify-between pb-5 border-b border-gray-50 dark:border-gray-700 last:border-0 last:pb-0">
                                  <div className="flex items-center space-x-4 min-w-0">
                                      <img src={item.avatar} className="w-11 h-11 rounded-2xl object-cover border border-gray-100 dark:border-gray-700 shadow-sm" alt="Avatar" />
                                  ...
                                  <button onClick={() => handleDeleteMember(item.id)} className="p-2.5 text-red-500 bg-red-50 dark:bg-red-900/20 rounded-2xl"><Trash2 size={16} /></button>
                              </div>
                            );
                        }
                                ...
                                <div key={item.id} onClick={() => { setPreviewPost(item); setIsCaptionExpanded(false); }} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-900/50 rounded-2xl border border-transparent hover:border-synergy-blue/30 transition-all cursor-pointer">
                                    <div className="flex items-center space-x-4 min-w-0">
                                        <div className="w-12 h-12 bg-black rounded-xl overflow-hidden shrink-0">
                                            {item.type === 'video' ? <Play size={16} className="text-white m-auto mt-4" /> : <img src={item.content} className="w-full h-full object-cover" alt="p" />}
                                        </div>
                                        <div className="min-w-0">
                            ...
                            return (
                                <div key={item.id} onClick={() => setSelectedKyc(item)} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-900/50 rounded-2xl border border-transparent hover:border-synergy-blue/30 transition-all cursor-pointer">
                                    <div className="flex items-center space-x-4">
                                        <img src={item.avatar} className="w-10 h-10 rounded-xl" alt="u" />
                                        <div>
                                            <p className="text-xs font-black text-gray-900 dark:text-white">{item.name}</p>
                                            <p className="text-[9px] text-gray-500">{item.email || `user_${item.id}@example.com`}</p>
                                        </div>
                                    </div>
                                    <div className="flex items-center space-x-2">
                                        <span className={`px-2 py-1 rounded-md text-[7px] font-black uppercase ${item.id % 3 === 0 ? 'bg-emerald-500 text-white' : 'bg-amber-400 text-white'}`}>
                                            {item.id % 3 === 0 ? 'Verified' : 'Pending'}
                                        </span>
                                    </div>
                                </div>
                            );
                          ...
                          <div className="space-y-2">
                              {selectedOrder.items.map((item, i) => (
                                  <div key={i} className="flex items-center justify-between p-3 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm">
                                      <div className="flex items-center space-x-3">
                                          <img src={item.image} className="w-10 h-10 rounded-lg object-cover" alt="p" />
                                          <div>
                  ...
                  <div className="p-6 bg-gray-50 dark:bg-gray-800/50 border-t border-gray-100 dark:border-gray-800 flex items-center space-x-3">
                      {selectedOrder.status === 'Pending' ? (
                          ...
                          <button 
                            ...
                            className="flex-1 py-4 bg-synergy-blue text-white rounded-[20px] font-black text-[9px] uppercase tracking-widest flex items-center justify-center space-x-2"
                          >
                            <Activity size={14} />
               ...
               <div className="bg-white dark:bg-gray-800 w-full max-w-sm rounded-[32px] overflow-hidden shadow-2xl relative z-10 animate-in zoom-in-95 border border-transparent dark:border-gray-700" onClick={(e) => e.stopPropagation()}>
                    <div className="relative aspect-square bg-gray-100 dark:bg-gray-900">
                        {previewPost.type === 'video' ? (
                            <video src={previewPost.content} className="w-full h-full object-cover" controls autoPlay muted loop />
                        ) : (
                            <img src={previewPost.content} className="w-full h-full object-cover" alt="content" />
                        )}
                        ...
                        <div className="flex justify-between items-start mb-4">
                           <div className="flex items-center space-x-2">
                               <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-synergy-blue to-purple-500 p-[2px] shadow-sm">
                                   <img src={previewPost.avatar} className="w-full h-full rounded-full border border-white dark:border-gray-800" alt="av" />
                               </div>
                               <div>
          ...
          <div className="fixed inset-0 z-[150] flex items-center justify-center p-6 bg-black/70 backdrop-blur-md animate-in fade-in duration-300" onClick={() => setSelectedKyc(null)}>
              <div className="bg-white dark:bg-gray-900 w-full max-w-sm rounded-[36px] overflow-hidden shadow-2xl animate-in zoom-in-95 relative border border-white/10" onClick={(e) => e.stopPropagation()}>
                  <div className="p-6 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center bg-gray-50 dark:bg-gray-800/50">
                      <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-gray-400 font-mono tracking-tighter">USER_{selectedKyc.id}</p>
                          </div>
                      </div>
                  ...
                  <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto no-scrollbar">
                      <div className="space-y-4">
                          <div className="flex items-center space-x-4">
                              <img src={selectedKyc.avatar} className="w-16 h-16 rounded-2xl object-cover border-2 border-white dark:border-gray-800 shadow-md" alt="Avatar" />
                              <div>
                                  <p className="text-lg font-black text-gray-900 dark:text-white tracking-tight">{selectedKyc.name}</p>
                                  <p className="text-xs text-gray-500 font-medium">{selectedKyc.email || `user_${selectedKyc.id}@example.com`}</p>
                              </div>
                          </div>
                          ...
                          <div className="bg-gray-50 dark:bg-gray-800/50 p-5 rounded-3xl border border-gray-100 dark:border-gray-700 space-y-4">
                              <div className="space-y-1">
                                  ...
                                  <div className="aspect-[1.6/1] bg-gray-200 dark:bg-gray-700 rounded-2xl overflow-hidden relative group">
                                      <img 
                                          src={`https://picsum.photos/seed/kyc_${selectedKyc.id}/600/400`} 
                  ...
                  <div className="p-6 bg-gray-50 dark:bg-gray-800/50 border-t border-gray-100 dark:border-gray-800 flex space-x-3">
                      <button 
                        onClick={() => { updateKycStatus('Verified'); setSelectedKyc(null); }}
                      ...
                      <button 
                        onClick={() => { updateKycStatus('Rejected'); setSelectedKyc(null); }}
