...
export const ProductDetail: React.FC = () => {
  const { products, addToCart, calculateCommission, user, referrer, addReferrer, t, setBottomNavHidden, cart } = useApp();
  ...
  const handleAddReferrer = () => {
      ...
      if (success) {
          ...
          setReferrerError("Invalid Referral Code");
      }
  };
    ...
    <div className="pb-24 pt-0 px-4 max-w-md mx-auto min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
      ...
      <div className="relative px-2 py-2 z-20">
            <div className="flex justify-between items-start mb-3">
                ...
                <div className="flex items-center space-x-1 bg-white dark:bg-gray-800 px-2.5 py-1 rounded-full border border-gray-100 dark:border-gray-700 shadow-sm shrink-0 mt-1">
                    ...
                    <span className="text-[10px] font-black text-gray-700 dark:text-gray-200">4.8</span>
                ...
                <div className="flex justify-between items-center">
                    <div className="inline-flex items-center space-x-3 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800/50 p-1 pr-3 rounded-2xl">
                        ...
                        <div className="flex flex-col">
                            ...
                            <p className="text-xs font-black text-emerald-700 dark:text-emerald-300">+฿{(commission ?? 0).toLocaleString()}</p>
                        </div>
                    </div>
                    ...
                    <div className="pb-2">
                        ...
                        <div className="flex space-x-4 overflow-x-auto no-scrollbar pb-4 -mx-2 px-2">
                            {relatedProducts.map(relProduct => {
                                const relFinalPrice = getDiscountedPrice(relProduct);
                                const relHasDiscount = relFinalPrice < relProduct.price && user?.tier !== UserTier.STARTER;

                                return (
                                    <div 
                                        key={relProduct.id}
                                        onClick={() => relProduct.stock > 0 && navigate(`/product/${relProduct.id}`)}
                                        className="w-40 shrink-0 bg-white dark:bg-gray-800 rounded-xl shadow-sm hover:shadow-md transition duration-300 cursor-pointer active:scale-[0.98] border border-transparent dark:border-gray-700 overflow-hidden flex flex-col relative"
                                    >
                                        ...
                                        <div className="p-3">
                                            <h3 className="text-sm font-bold text-gray-800 dark:text-gray-100 line-clamp-1 mb-1">{relProduct.name}</h3>
                                            <div className="flex justify-between items-end">
                                                <div className="min-w-0">
                                                    <div className="flex items-center space-x-1.5">
                                                        <p className="text-base font-black text-synergy-blue">฿{(relFinalPrice ?? 0).toLocaleString()}</p>
                                                        {relHasDiscount && (
                                                            <p className="text-[10px] text-gray-400 line-through leading-none">฿{(relProduct.price ?? 0).toLocaleString()}</p>
                                                        )}
                                                    </div>
                                                    <div className="flex items-center space-x-1 mt-1">
                                                        ...
                                                        <p className="text-[9px] text-gray-400 font-bold whitespace-nowrap">
                                                            {formatSold(relProduct.sold)} {t('home.sold')}
                                                        </p>
                                                    </div>
                                                </div>
                                                <button 
      ...
      <div className="fixed bottom-0 left-0 right-0 z-[100] bg-white/95 dark:bg-gray-900/95 backdrop-blur-2xl border-t border-gray-100 dark:border-gray-800 pb-safe shadow-[0_-10px_40px_rgba(0,0,0,0.08)] rounded-t-2xl overflow-hidden">
          <div className="max-w-md mx-auto p-4 flex space-x-3">
              ...
              >
                  <span>{product.stock <= 0 ? 'Sold Out' : `Buy ฿${(finalPrice ?? 0).toLocaleString()}`}</span>
