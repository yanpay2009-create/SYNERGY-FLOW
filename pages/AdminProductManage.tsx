...
export const AdminProductManage: React.FC = () => {
  ...
  const handleAddDescriptionImage = (event: React.ChangeEvent<HTMLInputElement>, product: Product) => {
    const file = event.target.files?.[0];
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => {
        ...
        updateProduct(product.id, { descriptionImages: [...currentImages, base64] });
        // Reset input value
        event.target.value = '';
      };
      reader.readAsDataURL(file);
                                ...
                                <div className="flex flex-col space-y-2 flex-1 ml-3">
                                    ...
                                    <div className="flex items-center space-x-1 bg-white dark:bg-gray-800 px-2 py-1 rounded-lg border border-indigo-100 dark:border-indigo-700">
                                        ...
                                        <input 
                                            ...
                                            onChange={(e) => updateProduct(product.id, { expiryDate: e.target.value ? new Date(e.target.value).toISOString() : undefined })}
