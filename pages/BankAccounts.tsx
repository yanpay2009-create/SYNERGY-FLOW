...
export const BankAccounts: React.FC = () => {
  const { user, kycStatus, bankAccounts, selectedBankId, selectBank, addBankAccount, removeBankAccount } = useApp();
  ...
  const handleAdd = (e: React.FormEvent) => {
    ...
    if (kycStatus !== 'Verified') {
        alert("Please complete identity verification (KYC) before adding a bank account.");
        return;
    }

    const kycName = user?.kycFullName || '';
    if (form.accountName.trim().toLowerCase() !== kycName.trim().toLowerCase()) {
        alert(`Account name must match your verified KYC name: ${kycName}`);
        return;
    }
