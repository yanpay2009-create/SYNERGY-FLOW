...
export const Auth: React.FC = () => {
  ...
  const { login, addReferrer, systemSettings, t, updateUserSecurity, language } = useApp();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isLogin) {
        ...
        if (!validatePassword(password)) {
            alert("Password must be at least 6 characters long and contain at least one English lowercase letter (a-z) and one number (0-9).");
            return;
        }
        if (!agreedToTerms) {
  ...
  const executeOtpVerification = (code: string) => {
      setIsVerifying(true);
      ...
      setTimeout(() => {
          ...
          if (isResetFlow) {
              ...
              // SUCCESS: Login and GO TO HOME IMMEDIATELY
  ...
  const handleResetPassword = (e: React.FormEvent) => {
      ...
      if (!validatePassword(newPassword)) {
          alert("Password must be at least 6 characters, including a lowercase letter and a number.");
          return;
      }
